...
export default function Sidebar({ onNavigate }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-screen overflow-y-auto flex flex-col shadow-lg lg:shadow-none">
      <div className="border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 py-3 sm:py-4 flex items-center">
              ...
              {item.children ? (
                <>
                  ...
                  {expandedItems.includes(item.name) && (
                    <div className="ml-9 mt-1 space-y-0">
                      {item.children.map((child) => {
                        if (child.isCategory) {
                          return (
                            <div key={child.path} className="px-3 py-1.5 mt-1">
      ...
      <div className="p-3 sm:p-4 border-t border-gray-200 dark:border-gray-800">
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 sm:p-4 text-center">
          <Download className="w-8 h-8 sm:w-10 sm:h-10 mx-auto mb-2 sm:mb-3 text-blue-600 dark:text-blue-400" />
          ...
          <button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-medium py-2 px-3 sm:px-4 rounded-lg transition-colors">
            DOWNLOAD NOW
          </button>
        </div>
      </div>
